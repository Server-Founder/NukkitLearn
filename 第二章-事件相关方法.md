...
### 本章目录
- [前言](#一、前言)
- [Nukkit&#32;插件初步配置](#二、Nukkit&#32;插件初步配置)
- [从玩家加入开始...](#三、从玩家加入开始...)
- [我不许你动&#32;你就不能动](#四、我不许你动&#32;你就不能动)
- [禁止其余动作](#五、禁止其余动作)
- [效果](#七、效果)
- [最后](#八、最后)
...
这些都会调用这个方法。我们可以根据判断：发起伤害的实体`getDamager()`和受到伤害的实体`getEntity()`，只要其中有一个是未登录的玩家，就取消本次Event。
...
先判断玩家是否已登录，然后通过`event.getMessage()`获得消息，实际上也就是密码，来做对比。如果错误进行提示，如果正确则将其加入已登录玩家的列表中。
